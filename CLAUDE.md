...
- `lightbox.ts` — behavior for the modal a project capture opens into. Imported from `Lightbox.astro`, which a project page renders once, so only those three pages ship it and `@panzoom/panzoom` (3.6 kB gzipped) is a dynamic import that nothing fetches until a capture is opened. **`Lightbox.astro` renders the slides; this file never builds them.** It used to, and that cost the whole feature its styles: markup built in JS has no element for a utility to sit on, so the rules had to live in a stylesheet of their own. Nothing about a capture is known only at runtime, so there was nothing to build. Most of it is the platform: `showModal()` for the focus trap, Escape, `inert` and `::backdrop`, and a `scroll-snap` rail for paging with real momentum and rubber-banding. Panzoom is only there for pinch. Progressive the same way `footnotes.ts` is: the frames render as plain `div`s and are upgraded to buttons at runtime, because a server-rendered button whose handler never arrives is a control that does nothing.
...
- **Panzoom reports a drag as its distance from where the gesture began, not as a delta.** A finger resting past a bound sends the same value on every frame, so anything that sums those frames grows while the reader holds still. `clampPan` reads how far past the bound it is fresh each time for that reason. Its `x` is also in the picture's own units, before the zoom multiplies them, so a threshold in finger pixels has to be scaled back up.
