...
**The URL is the `slug`, not the filename.** An optional `slug` in blog frontmatter owns the address, so a folder keeps its date prefix and sorts chronologically while the URL carries none: `/writing/espresso-machines/`. All five posts set one. The glob loader applies it: `entry.id` is the `slug` when there is one, which is why `params: { slug: post.id }` and `postHref()` in `src/lib/content.ts` need no `??` of their own. It reads raw frontmatter, before the schema runs, so the zod rule (one segment, in lower case, dashes between words) fails a bad slug rather than gating the id. Ordering never reads either one.
...
**Two posts with one slug is a build failure.** Astro only warns on a duplicate id and keeps the last of the two, so the earlier post loses its page while the build stays green; that was measured, not assumed. `scripts/og-card.ts` refuses it, because that script is the only reader of the content directory outside Astro and by the time a route runs the loader has already dropped one. It counts drafts, since a draft is what a preview build renders.
