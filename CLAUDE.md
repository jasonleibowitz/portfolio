...
**The URL is the `slug`, not the filename.** An optional `slug` in blog frontmatter owns the address, so a folder keeps its date prefix and sorts chronologically while the URL carries none: `/writing/espresso-machines/`. The glob loader applies it: `entry.id` is the `slug` when there is one, which is why `params: { slug: post.id }` and `postHref()` in `src/lib/content.ts` need no `??` of their own. It reads raw frontmatter, before the schema runs, so the `address` rule in `src/content.config.ts` (one segment, in lower case, dashes between words) fails a bad slug rather than gating the id. Ordering never reads either one.

**A post keeps every address it has ever had, in `aliases`.** All five posts moved off their dated URLs, and each lists the old one. `scripts/redirects.ts` turns each alias into a 301 in `dist/_redirects`, under both `/writing/` and `/blog/`. The old address lives with the post for the same reason its images do: deleting the post takes its redirects and renaming it carries them, neither of which a list in `astro.config.mjs` can do. **Add to `aliases`, never remove from it** — a removed alias is a link somewhere on the internet that stops working.

**Slug and alias share one namespace, and a collision is a build failure.** Astro only warns on a duplicate id and keeps the last of the two, so the earlier post loses its page while the build stays green; that was measured, not assumed. `scripts/content.ts` refuses it, because these scripts are the only readers of the content directory outside Astro and by the time a route runs the loader has already dropped one. It counts drafts, since a draft is what a preview build renders, and it counts an alias as a claim on an address: a post that took another post's retired address would hijack a live redirect. A post listing its own slug as an alias is refused too, since that redirect points at itself.

**`slugify()` in `src/lib/slug.ts` derives a slug from a title**, and the generator (and later the admin) offers the result. It is four lines of JavaScript, deliberately not `github-slugger`: that package slugs heading anchors, so it keeps accents and doubled dashes, which the `address` rule refuses. Measured on 8 titles, it failed the rule on 4; `slugify()` failed on none. Do not swap plop's `dashCase` back in either, since it splits camelCase into `e-sim` and `i-phone`.
...
**The hero is not the share card.** `scripts/og-card.ts` composes a 1200x630 card per post, and `shareCard()` fails the build if one is missing. It reads frontmatter itself, outside Astro, through `scripts/content.ts`: `entryFiles()` accepts both `<name>.mdx` and `<name>/index.mdx`, `published()` then lets a `slug` win over the filename, and between them they must keep deriving the same ids as the glob loader above. An id that disagrees names a card no page asks for, and leaves the card a page does ask for undrawn. `scripts/redirects.ts` reads through the same module, so a card and a redirect can never disagree about where a post lives.
...
**`dist/_redirects` is generated the same way**, by `scripts/redirects.ts` as the last step of `pnpm build`, from the `aliases` of each post. Cloudflare answers those at the edge with a real 301, which Astro's own `redirects` cannot do: those emit an HTML page carrying `<meta http-equiv="refresh">`, which costs a download and a render first. `/blog/*` stays in `astro.config.mjs`, because it covers tag archives and any path no post claims. Redirects in `_redirects` do not apply to a request answered by Worker code, which does not matter here: `wrangler.jsonc` declares no `main`, so every request is a static asset request.
